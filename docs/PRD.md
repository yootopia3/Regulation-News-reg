...
- **Logic**: **Conditional Alerting**.
    - ✅ **Send Alert**: If Article passed Tier 2 Analysis (Status `ANALYZED`).
    - 🔇 **Silent**: If Article was filtered at Tier 1 (Status `SKIPPED`).
    - 🚨 **System Critical**: Watchdog failure or Pipe break.
...
> Admin fixes the selector in `agencies.json` via GitHub Mobile. System restores itself.
