'use client'
import SanctionBriefing from './SanctionBriefing'
...
export default function SanctionReportModal({ article, onClose }: { article: Article; onClose: () => void }) {
    ...
    return <div className="fixed inset-0 z-[80] flex items-center justify-center p-3 sm:p-6">
        ...
        <div ref={dialog} role="dialog" aria-modal="true" aria-labelledby="sanction-report-title" className="relative flex flex-col w-full max-w-4xl max-h-[90dvh] rounded-2xl bg-white border border-slate-200 shadow-2xl overflow-hidden text-slate-900"
            ...
            <header className="flex items-center justify-between gap-4 border-t-4 border-cyan-400 bg-slate-900 px-5 py-5 text-white sm:px-8">
                <div className="flex items-center gap-3"><span className="bg-blue-50 text-blue-600 p-2 rounded-xl"><Sparkles className="w-5 h-5" /></span><div><p className="text-sm font-bold">AI 심층 보고서</p><p className="text-xs text-slate-300 mt-1">제재공시 브리핑 · 당행 사고예방 점검</p></div></div>
                <button ref={close} onClick={onClose} aria-label="보고서 닫기" className="rounded-full p-2 text-slate-200 hover:bg-slate-700"><X className="w-5 h-5" /></button>
            </header>
            <div className="overflow-y-auto px-5 py-6 sm:px-8 sm:py-8">
                ...
                {state === 'ready' && report && <>
                    ...
                    <SanctionBriefing report={report.report} />
                </>}
            </div>
