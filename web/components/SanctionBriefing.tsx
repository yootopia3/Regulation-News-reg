...
export default function SanctionBriefing({ report }: { report: PublicationReport }) {
    return <div className="mt-6 space-y-6 text-slate-800">
        <section aria-label="제재공시 요약 브리핑">
            <h2 className="mb-4 flex items-center gap-3 text-lg font-bold"><span className="rounded-lg bg-sky-50 px-2 py-1 text-xs text-sky-800">01</span>제재공시 요약 브리핑</h2>
            <div className="space-y-4">{report.items.map(item => <div key={item.finding_id}>
                <h3 className="mb-2 font-semibold"><span className="mr-2 rounded bg-amber-50 px-2 py-1 text-xs text-amber-800">지적사항</span>{item.title}</h3>
        ...
        <section aria-label="당행 관점 사고예방 점검" className="border-t border-slate-200 pt-6">
            <h2 className="mb-4 flex items-center gap-3 text-lg font-bold"><span className="rounded-lg bg-sky-50 px-2 py-1 text-xs text-sky-800">02</span>당행 관점 사고예방 점검</h2>
            <div className="space-y-5">{report.items.map(item => <div key={item.finding_id}>
                {report.items.length > 1 && <h3 className="mb-3 font-semibold">{item.title}</h3>}
        ...
        <section aria-label="점검 포인트" className="border-t border-slate-200 pt-6">
            <h2 className="mb-4 flex items-center gap-3 text-lg font-bold"><span className="rounded-lg bg-sky-50 px-2 py-1 text-xs text-sky-800">03</span>점검 포인트</h2>
            {report.items.map(item => <div key={item.finding_id} className="mb-4">
                {report.items.length > 1 && <h3 className="mb-3 font-semibold">{item.title}</h3>}
