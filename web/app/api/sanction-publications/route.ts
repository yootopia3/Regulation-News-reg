...
export async function GET(request: NextRequest) {
    ...
    const query = z.object({ offset: z.coerce.number().int().min(0).max(1000000), id: z.string().uuid().optional(), format: z.enum(['json','xlsx']) }).safeParse({
    ...
    try {
        const { offset, id, format } = query.data
        const reports = await publishedReports(adminClient(), id ? 0 : offset, id)
        if (format === 'xlsx') {
        ...
        return NextResponse.json({ reports, enabled: true, nextOffset: !id && reports.length === 20 ? offset+20 : null }, { headers: PRIVATE_HEADERS })
