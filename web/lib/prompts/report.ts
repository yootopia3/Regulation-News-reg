...
export function buildReportPrompt({ agency, title, content }: ReportPromptInput): string {
  return `
        ...
        # **절대 규칙**
        - 아래 "입력 자료"에 포함된 내용만 사용하라
        - 외부 지식, 과거 사례, 일반론, 배경 설명을 절대 추가하지 마라
        - 입력 자료에 언급되지 않은 수치, 날짜, 사건, 기관명, 인용을 절대 생성하지 마라
        - 입력 자료에 없는 전망, 파급효과, 제언을 작성하지 마라
        ...
        4. **제언/파급효과 금지**: 입력 자료에 명시적으로 포함된 경우에만 정리하고, 그 외에는 이 섹션을 작성하지 마라
