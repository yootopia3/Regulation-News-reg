...
spawns a Codex exec session with the phase prompt, and updates status.
...
def git_commit_phase(task_name: str, task_dir_name: str, phase_num: int, phase_name: str, gh_env: dict[str, str]) -> bool:
    """Two-step commit: agent fallback (if needed) + runner housekeeping."""
    ...
    # --- Step 1: agent fallback commit (code changes Codex didn't commit) ---
    git_run("add", "-A")
    # Unstage runner-generated files so they don't mix into the agent commit
...
7. 작업 중 사용자 개입이 반드시 필요한 상황(API key 제공, 외부 서비스 인증, 수동 설정 등)이 발생하여 직접 해결이 불가능하다면:
   ...
   - "blocked_reason" 필드에 사유를 구체적으로 기록하세요 (예: "외부 서비스 인증 정보가 설정되지 않음").
   - 작업을 즉시 중단하세요. 해결을 시도하지 마세요.
8. 단일 파일은 200줄을 넘기지 마세요. 넘어가면 파일을 분리하세요.
...
def run_phase(task_dir: Path, phase: dict, preamble: str, gh_env: dict[str, str]) -> dict:
    ...
    extra_args = shlex.split(os.environ.get("CODEX_EXEC_ARGS", ""))
    cmd = [
        codex_bin,
        "exec",
        "--full-auto",
...
def main():
    ...
    push_on_complete = bool(index.get("push_on_complete", True))
    ...
    print(f"  Push on complete: {push_on_complete}")
    print(f"{'='*60}")
    ...
    while True:
        ...
        # Re-read index.json to check what Codex did
        fresh_index = load_index(index_file)
        status = None
    ...
    # Push branch to remote only when requested by the task index.
    branch = f"feat-{task_name}"
    if push_on_complete:
        ...
        print(f"  Push skipped (push_on_complete=false). Branch is local: {branch}")
