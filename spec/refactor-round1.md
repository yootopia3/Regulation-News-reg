...
AC 는 두 층으로 나눈다. **자동 검증**은 runner/phase 7 에서 판정하고, **수동
회귀 검증**은 `.env` + 외부 서비스가 필요하므로 사용자가 별도로 수행한다.
task status 는 자동 검증 통과 시점에 `completed` 로 간주하되, 수동 회귀 검증이
끝날 때까지는 **`regression-report.md`의 "수동 검증 대기" 항목이 모두 해소되어야
"라운드 1 종결"** 로 본다.

### 6.1 자동 검증 AC (runner / phase 7 판정)

- `python -c "from src.pipeline import Pipeline"`, `from src.services.analyzer
  import HybridAnalyzer` 둘 다 사이드 이펙트 없이 통과.
- agency 코드 문자열 리터럴 0건 (`src/pipeline.py` 에 enum/상수만).
- DB 스키마 diff 0건.
- `web/`, `config/agencies.json`, `.github/`, `scripts/`, `db/`, 루트 dump 파일
  (`./debug_*.md`, `./debug_*.py`, `./*.txt`, `./agency_stats.json`) diff 0건.
- 분석 결과 JSON 키 셋 동일 (코드 검토 기준).

### 6.2 수동 회귀 검증 AC (사용자가 `.env` 환경에서 수행)

- `pip install -r requirements.txt` 성공 (runner 실행 전 1회 수동, §0 참조).
- `python src/main.py` 1사이클 정상 종료, 9개 agency 처리 로그 확인.
- 새 article 1건 이상 DB 저장 확인.
- Telegram 알림 포맷이 기존과 동일.
- 제재 중복 체크가 동일 케이스(`examMgmtNo`+`emOpenSeq`)를 여전히 필터링.
